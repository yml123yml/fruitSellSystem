...
export default {
  ...
  methods: {
    ...
    handleDiff (obj1, obj2) {
      var isSame = true
      if (obj1.id === obj2.id && obj1.chooses.length === obj2.chooses.length) {
        for (let i = 0; i < obj1.chooses.length; i++) {
          if (obj1.chooses[i] !== obj2.chooses[i]) {
            isSame = false
          }
        }
        console.log(isSame)
    ...
    handleToCart () {
      ...
      pro.choose = []
      pro.choose.push(this.choose[this.cartLen])
      var cartFlag = false
      if (!cartPros) {
        cartPros = []
      } else {
        cartPros.forEach(cartPro => {
          if (this.handleDiff(cartPro, pro)) {
            cartPro.num = cartPro.num + 1
          }
        })
      }
      if (!cartFlag) {
        pro.cartId = parseInt(cartId)
        cartPros.push(pro)
        cartId = parseInt(cartId) + 1
      }
      cartPros = JSON.stringify(cartPros)
      localStorage.setItem('cartPros', cartPros)
