<template>
  <div>
    <form>
      ...
      <input type="search" @click="onSearch" v-model="search" ref="searchInput" />
    </form>
...
export default {
  ...
  methods: {
    onSearch () {
      ...
      if (search) {
        this.searchData = this.allfruit.title.filter(value => value.title.indexof(this.search) !== -1)
        console.log('dfjdljoweurioennd')
